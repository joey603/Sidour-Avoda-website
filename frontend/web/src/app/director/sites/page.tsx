...
import {
  AUTO_WEEKLY_WORKER_CHANGES_STORAGE_KEY,
  clearPlanningV2MultiSiteFilterKeysForWeek,
  clearWeeklyPlanLocalStorageKeysForWeekAndSites,
...
export default function SitesList() {
  ...
  async function onTestAutoPlanningNow() {
    ...
    try {
      ...
      clearPlanningV2MultiSiteFilterKeysForWeek(result.target_week_iso);
      const clearedSiteIds = sites.map((s) => Number(s.id)).filter((id) => Number.isFinite(id) && id > 0);
      clearWeeklyPlanLocalStorageKeysForWeekAndSites(
