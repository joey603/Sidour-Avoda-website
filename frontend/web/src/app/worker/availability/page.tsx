...
export default function WorkerAvailabilityPage() {
  ...
  // Charger les זמינות sauvegardées pour la semaine prochaine
  function loadSavedAvailability(siteId: number) {
    if (!nextWeekStart) return;
    const key = getWeekKey(siteId, nextWeekStart);
    try {
  ...
  useEffect(() => {
    if (selectedSiteId && nextWeekStart) {
      loadSavedAvailability(selectedSiteId);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedSiteId, nextWeekStart]);
  ...
  async function loadSiteInfo(siteId: number) {
    try {
      ...
      // Charger les זמינות sauvegardées pour cette semaine (cela définit aussi isEditing et success)
      loadSavedAvailability(siteId);
    } catch (e: any) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const weekKey = getWeekKey(selectedSiteId, nextWeekStart);
